...
Mlesac::Mlesac() {
  ...
  fMlesacMaxIteration = 500;
	fMlesacThreshold = 30;//30;  //min number of points
	fMlesacPointThreshold = 0.10;//0.07;
	fMlesacChargeThreshold = 0.10;//0.07;
	fMlesacDistance = 15;//7;
}
...
void Mlesac::Solve(double dist_thres, double Nminpoints, int Nintera)
{
    ...
    fMlesacDistance = dist_thres;
    fMlesacThreshold = Nminpoints;
    ...
    remainIndex.push_back(i);
      ...
      double sigma = fMlesacDistance/1.96;
      ...
      std::vector< std::pair <double,int> > IdxMod2;
        ...
        if(remainIndex.size()<fMlesacThreshold) break;
